      ...
      db.query("SELECT data FROM config WHERE name = $1;", ["user.role." + key], function (err, rows) {
        ...
        if (rows.length > 0) {
          rows.forEach(function (row) {            
            ...
            var permissionsArray = Object.keys(permissionsObject).map(i => permissionsObject[i]);
...
function access(permission, account, callback) {
  ...
  if (account.permissions) {
    findPermission(account.permissions);
  ...
  function findPermission(permissions) {
    for(i = 0; i < permissions.length; i++) {
      Object.keys(permissions[i]).forEach(function(key) {
        if (permissions[i][key] == permission) {
